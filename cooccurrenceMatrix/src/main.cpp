...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cout << "./enumeratePairs <outputFileName> <numberOfThreads> [input files] ..." << std::endl;
        exit(0);
    }
    ...
    for (int n = 3; n < argc; n++) {
        ...
        for (int i =0; i<thread_num; i++) {
            threads.emplace_back(generatePairs, std::ref(freq_map), std::ref(events), i, thread_num, window, std::ref(rowCounter));
        }
